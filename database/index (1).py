...
if missing_vars:
    # In a serverless environment, we can't stop the process with sys.exit,
    # but we can log the fatal error. The app will likely fail on subsequent calls.
    print(f"FATAL: Missing required environment variables: {', '.join(missing_vars)}")
    # sys.exit(1) # This is not ideal for Vercel, logging is preferred.
...
try:
    ...
    # sys.exit(1) # Avoid exiting in serverless.
...
# NOTE: BackgroundScheduler is not suitable for Vercel's serverless environment.
# The functionality to delete messages after a delay has been removed.
# You can explore Vercel Cron Jobs for similar scheduled tasks.
...
<title>PmwBDino - Your Entertainment Hub</title>
<style>
...
{# === MODIFICATION START === #}
{# শুধুমাত্র Latest Movies এবং Latest Series হোমপেজে দেখানো হবে #}
...
<div class="admin-container">
    ...
    <form method="post" action="{{ url_for('admin') }}">
        <fieldset>
            ...
            <div class="form-group"><label for="languages">Languages (comma separated):</label><input type="text" name="languages" id="languages" placeholder="e.g. Hindi, English" /></div>
            <!-- [MODIFIED] Added Release Date and Rating fields -->
            <div class="form-group"><label for="release_date">Release Date (YYYY-MM-DD):</label><input type="text" name="release_date" id="release_date" placeholder="e.g., 2024-08-17" /></div>
            <div class="form-group"><label for="vote_average">Rating (out of 10):</label><input type="number" name="vote_average" id="vote_average" step="0.1" min="0" max="10" placeholder="e.g., 8.5" /></div>
...
<script>
    ...
    async function fetchFromTmdb() {
        ...
        try {
            ...
            // [MODIFIED] Populate new fields from TMDB
            document.querySelector('form [name="release_date"]').value = data.release_date || '';
            document.querySelector('form [name="vote_average"]').value = data.vote_average || '';
...
<div class="admin-container">
  ...
  <form method="post">
    <fieldset>
        ...
        <!-- [MODIFIED] Added Release Date and Rating fields -->
        <div class="form-group"><label>Release Date (YYYY-MM-DD):</label><input type="text" name="release_date" value="{{ movie.release_date or '' }}" placeholder="e.g., 2024-08-17" /></div>
        <div class="form-group"><label>Rating (out of 10):</label><input type="number" name="vote_average" value="{{ movie.vote_average or '' }}" step="0.1" min="0" max="10" placeholder="e.g., 8.5" /></div>
...
# [MODIFIED] This function is updated to correctly handle URL encoding.
@app.route('/category/<cat_name>')
def movies_by_category(cat_name):
    # This unquote function will decode URL parts like '%20' back into spaces.
    decoded_cat_name = unquote(cat_name)
    title = decoded_cat_name.replace("_", " ").title()
...
def admin():
    if request.method == "POST":
        # [MODIFIED] Added release_date and vote_average to the data being saved.
        vote_average_str = request.form.get("vote_average")
        movie_data = {
            ...
            "vote_average": float(vote_average_str) if vote_average_str else 0.0,
            ...
            "links": [], "files": [], "episodes": [], "season_packs": [], "streaming_links": []
        }
        
        if not movie_data.get('tmdb_id'):
            tmdb_details = get_tmdb_details_from_title(movie_data['title'], movie_data['type'])
            if tmdb_details:
                # Prioritize manually entered data over TMDB data if both exist
                manual_data = {k: v for k, v in movie_data.items() if v}
                tmdb_details.update(manual_data)
...
def edit_movie(movie_id):
    ...
    if request.method == "POST":
        ...
        # [MODIFIED] Added release_date and vote_average to the data being updated.
        vote_average_str = request.form.get("vote_average")
        update_data = {
            "title": request.form.get("title", "").strip(), 
            "type": content_type,
            ...
            if len(parts) > 1:
                try:
                    ...
                    if message_to_copy_id:
                        ...
                        # Vercel Limitation: Cannot use background scheduler to delete messages.
                        # The following lines are removed. The message will not be auto-deleted.
                        # if res.get('ok'):
...
# Note: The if __name__ == "__main__": block is removed as Vercel handles the server.
